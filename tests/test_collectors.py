...
def test_collect_sources_honors_source_filter() -> None:
    ...
    items, errors = collect_sources(catalog, ScanRequest(sources_requested=["nvd"]), client=client)

    assert items == []
    assert errors == []


def test_collect_sources_continues_after_source_http_error() -> None:
    def handler(request: httpx.Request) -> httpx.Response:
    ...
    nvd_source = _source("nvd", "NVD CVEs", "nvd")
    nvd_source.default_order = 2
    client = httpx.Client(transport=httpx.MockTransport(handler))

    items, errors = collect_sources(
        catalog.model_copy(update={"sources": [blocked_source, nvd_source]}),
        ScanRequest(),
        client=client,
    )
    ...
    assert items == []
    assert len(errors) == 1
    assert errors[0].startswith("blocked_rss:")
