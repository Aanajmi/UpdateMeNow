...
def test_bare_umn_uses_packaged_defaults_without_local_config(monkeypatch) -> None:
    def fake_run_scan(request, sources, keywords):
        assert not Path("config/sources.yaml").exists()
        assert "cisa_kev" in [source.id for source in sources.enabled_sources]
    ...
    assert result.exit_code == 0
    assert "UpdateMeNow Cyber Update Scan" in result.stdout
    assert "Config is invalid." not in result.stdout
