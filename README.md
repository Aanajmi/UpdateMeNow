...
### A Source Shows `403 Forbidden`

Some public feeds occasionally block command-line clients or specific networks. UpdateMeNow now sends browser-like HTTP headers and does not stop the whole scan if one source blocks the request. The report continues with the sources that worked and prints a source warning.

To see which sources are reachable from your computer:

```bash
umn sources test
```

If one source is blocked on your network, you can still scan the others, for example:

```bash
umn scan --source cisa_kev --source nvd --source github
```

Or edit `config/sources.yaml` after running `umn config init` and set that source to `enabled: false`.
